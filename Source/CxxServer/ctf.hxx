...
class ctf_protocol : public protocol
{
  public:
     ...
     */
    ctf_protocol(std::uint8_t max_players)
        : protocol(max_players)
        , m_teams { team_type::a, team_type::b }
    {
    }
    ...
    void on_kill(connection & killer, connection & victim, kill_type type, std::uint8_t respawn_time) override
    {
        ...
        if (victim.team() != team_type::spectator)
        {
            ...
            {
                std::cout << "[  LOG  ]: player " << victim.name() << " dropped intel (reason: killed by " << killer.name() << ")" << std::endl;
            }
        }
    }
    ...
    void on_disconnect(connection & connection) override
    {
        if (connection.team() != team_type::spectator)
        {
            ...
            {
                std::cout << "[  LOG  ]: player " << connection.name() << " dropped intel (reason: disconnected)" << std::endl;
            }
        }
    }
