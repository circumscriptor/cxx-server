...
class server_handler : public connection_manager
{
  public:
     ...
     */
    server_handler(std::uint8_t max_players)
     ...
     */
    virtual void on_block_action(connection & source, block_action_type action, std::uint32_t x, std::uint32_t y, std::uint32_t z)
    {
        switch (action)
        {
     ...
     * @param name Name
     */
    virtual void on_existing_player(
        connection &     source,
        team_type        team,
        weapon_type      weapon,
        tool_type        tool,
