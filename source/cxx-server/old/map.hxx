...
class map
{
  public:
    ...
    [[nodiscard]] bool is_surface(std::uint32_t offset) const
    {
        ...
        // check above, below
        auto z = offset & 0x3F;
        if (z > 0 && !is_block(offset - 1))
        { // check block above
            return true;
        }
        if (z + 1 < size_z && !is_block(offset + 1))
        { // check block below
            return true;
        }
        // check left, right
        auto x = (offset >> 6) & 0x1FF;
        if (x > 0 && !is_block(offset - size_z))
        { // check block on left
            return true;
        }
        if (x + 1 < size_x && !is_block(offset + size_z))
        { // check block on right
            return true;
        }
        // check up, down
        auto y = (offset >> 15) & 0x1FF;
        if (y > 0 && !is_block(offset - size_yz))
        { // check block left
            return true;
        }
        if (y + 1 < size_y && !is_block(offset + size_yz))
        { // check block right
            return true;
        }
        return false;
